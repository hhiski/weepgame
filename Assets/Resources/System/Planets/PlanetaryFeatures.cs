...
class PlanetaryFeatureFormation
{
    ...
    public readonly PlanetaryFeature StratosphericIceClouds = new PlanetaryFeature { Name = "Stratospheric Ice Clouds", WaterSource = "Water Vapor" };
    public readonly PlanetaryFeature PolarIceCaps = new PlanetaryFeature { Name = "Polar Ice Caps", Rarity = 1, WaterSource = "Polar Ice Caps" };
    public readonly PlanetaryFeature Oceans = new PlanetaryFeature { Name = "Saline Oceans", Rarity = 1, WaterSource = "Sea Water" };
    public readonly PlanetaryFeature FreshWater = new PlanetaryFeature { Name = "Fresh Water Sources", Rarity = 1, WaterSource = "Fresh Water Sources" };


    internal PlanetaryFeature[] GetHydroFeatureList()
    {
        PlanetaryFeature[] features = new PlanetaryFeature[] {
            WaterVaporTraces,StratosphericIceClouds,PolarIceCaps,Oceans,FreshWater
        };
        return features;
    }
        ...
        foreach (PlanetaryFeature feature in featureList)
        {
            if (atmosphere.Temperature >= feature.TemperatureRange[0] && atmosphere.Temperature <= feature.TemperatureRange[1])
            {
                if (atmosphere.Pressure >= feature.PressureRange[0] && atmosphere.Temperature <= feature.PressureRange[1])
                {
                    ...
                    {
                        if (biosphere.BiosphereLevel >= feature.BiosphereLevelRange[0] && biosphere.BiosphereLevel <= feature.BiosphereLevelRange[1])
                        {
                            if (hydrosphere.PrimaryWaterSource.Name == feature.WaterSource || feature.WaterSource == "Not Set")
                            {
